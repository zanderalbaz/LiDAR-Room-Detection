...
def processSet(pb_set): #process points from csv
...
def processPoints(p_list):
    points = []
    for ps in p_list:
        points.append(processPoints_(ps)) #helper process function
    ...
    for point_batches, label in data:
        ...
        for point_set in point_batches:
            ...
            for _ in range(2):  # augment each point set 2 times
                ...
                #add both augmehtations to our point set
...
for i in range(len(points)): 
    #get each point batch (5 collected each interval)
    ...
    #process combined point batches
...
#combine points with one-hot label
...
#used shuffle=False and random_state=17 for re-producable results
...
def create_3d_cnn(input_shape, num_classes): 
    model = Sequential([
        Reshape((100, 10, 1), input_shape=input_shape),  #reshape tensor from 500x2x1 to 100x10x1
        Conv2D(32, (3, 3), padding="same", activation="relu"), #increase Z dim
        BatchNormalization(),
        Dropout(0.5),
        MaxPooling2D(pool_size=(2, 2)),  #pool to 50x5x1
        Conv2D(16, (3, 3), padding="same", activation="relu"),
        BatchNormalization(),
        Dropout(0.5),
        MaxPooling2D(pool_size=(2, 1)),# pool to 25x5x1
        Conv2D(8, (3, 3), padding="same", activation="relu"),
        BatchNormalization(),
        Flatten(), #start output layers
...
def train_3d_cnn(train_data, test_data, input_shape, num_classes):
    ...
    train_instances = []
    test_instances = []
    #seperate 5x2x500  batches into 5 2x500 batches
